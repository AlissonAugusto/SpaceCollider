...
public class RedController : MonoBehaviour
{
    ...
    {
        v = true;
        redParticle.GetComponent<ParticleSystem>().transform.rotation = Quaternion.Euler(new Vector3(Random.Range(0.0f, 360.0f), 90.0f, 0.0f));
    ...
    void Move()
    {
        ...
        {
            transform.position = Vector2.MoveTowards(transform.position, new Vector2(0, 0), velocity * Time.deltaTime);
        }
    }
    void OnMouseDown()
    {
        if (CircleController.gameState == 1)
        {
            ...
            Sounds.explosion = 1;
            v = false;
            redParticle.GetComponent<ParticleSystem>().Play();
    ...
    void Stoping()
    {
        redParticle.GetComponent<ParticleSystem>().Stop();
    }
    void Destroy()
    {
        Destroy(gameObject);
